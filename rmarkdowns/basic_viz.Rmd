...
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y=goal_10_score)) +
  geom_point()
```
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_10_score, color = goal_7_score)) +
  geom_point()
...
Interpretation:
You can see that in the dense cluster of countries in the top right hand quadrant show that those who have successfully dealt with poverty(SDG 1). That cluster also has many light blue shades (SDG 7) which indicates high energy scores. From that you can tell the country's poverty and energy usage go hand in hand. 

In the lower left quadrant you can see a spread of countries who aren't as successful in dealing with poverty and reducing inequalities (SDG10). In addition, their dark color also indicate that the also exhibit low clean energy scores.
...
```{r}
mena_con_data <- sdr_data %>%
  filter(regions_used_for_the_sdr == "MENA")

```

```{r}
ggplot(mena_con_data, aes(x = goal_1_score, y = country)) +
  geom_bar(stat = 'identity')
```
...
OECD falls very high in the successful poverty score (SDG1). The Sub-Saharan Africa has a very wide spread of score in comparison with other counties leaving more towards the higher end.
