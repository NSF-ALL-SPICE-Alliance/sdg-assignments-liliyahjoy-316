...
```{r, warning=FALSE}
ggplot(sdr_data, aes(x = goal_7_score, y = goal_1_score)) +
  geom_point() +
  theme_minimal() 
```
...
```{r}
p <- ggplot(sdr_data, aes(x = goal_11_score, y = goal_13_score, label = country)) +
  geom_point() +
...
The plot shows a negative correlation between SDG 11 (sustainable cities and comunities) and SDG 13 (climate action). This suggest that countries more developed sustainable urban infrastructure tend to not have a very good climate action. This correlation might make sense when you think about all the all these infrastructure often having higher per capita of carbon emission, energy consumption, and industrial activity. in comparison to lesser developed countries who don't have the tool nor system to use that much energy. Often emitting less and consuming less resources.

There where some outlining countries like Brunei, Kuwait, Oman, Australia, and etc. that exhibit high SDG 11 scores but fall very short in SGD 13 scores. One notable points where Qatar has a low SDG 11 score of 57 and a zero SDG 13 score, making it an extreme outline.
